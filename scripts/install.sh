...
if [ $? -ne 0 ]; then
  echo -e "${FG_C}${EBG_C} ERROR ${NO_C} Drush is required to install Vactory. Please install Vactory using Composer."
  exit 5
fi
DOCROOT="web"

echo "-----------------------------------------------"
echo " Installing Vactory CMS for local usage "
echo "-----------------------------------------------"
echo -e "${FG_C}${BG_C} EXECUTING ${NO_C} $DRUSH site-install vactory_decoupled --verbose --yes \\
...
$DRUSH site-install vactory_decoupled --verbose --yes \
  --root=$DEST_DIR/$DOCROOT \
  --db-url=$DB_URL \
  --site-mail=$SITE_MAIL \
  --account-mail=$ACCOUNT_MAIL \
...
if [ $? -ne 0 ]; then
  echo -e "${FG_C}${EBG_C} ERROR ${NO_C} The Drupal installer failed to install Vactory CMS."
  exit 3
fi
...
echo -e "${FG_C}${BG_C} 🎉 ${NO_C} Vactory CMS was installed! Your admin password is: $ACCOUNT_PASS\n\n"
